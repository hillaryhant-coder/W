...
async function sendOne(accountId, target, text) {
  ...
  // TikTok/Unicode gizli karakterleri temizle
  const cleanName = target.replace(/[\u200e\u200f\u202a-\u202e\u2066-\u2069]/g, '').trim();
  ...
  if (/^https?:\/\//i.test(target)) {
    ...
    const opened = await page.evaluate((name) => {
      ...
      const nodes = [...document.querySelectorAll('a, div, span, p, li')];
      for (const el of nodes) {
        const t = norm((el.innerText || el.textContent || '').split('\\n')[0]);
        if (!t || t.length > 100) continue;
        if (t === want || t.includes(want) || want.includes(t)) {
          const clickable = el.closest('a') || el.closest('[role="listitem"]') || el.closest('[class*="Item"]') || el;
          try { clickable.click(); return true; } catch (_) {}
        }
      }
      return false;
    }, cleanName).catch(() => false);

    if (!opened) {
      // kismi eslesme: ilk kelime
      const first = cleanName.split(',')[0].trim();
      await page.evaluate((name) => {
        const norm = (s) => String(s || '').toLowerCase();
        const want = norm(name);
        for (const el of document.querySelectorAll('a, div, span')) {
          const t = norm((el.innerText || '').split('\\n')[0]);
          if (t && want && t.includes(want.slice(0, 12))) {
  ...
  // 2) Mesaj kutusunu bul (birden fazla deneme)
  async function findInput() {
    return page.evaluate(() => {
      ...
      // gorunur contenteditable
      const all = [...document.querySelectorAll('[contenteditable="true"], [contenteditable="plaintext-only"], textarea')];
      for (const el of all) {
        if (el.offsetParent !== null && el.getBoundingClientRect().height > 10) {
          el.setAttribute('data-tt-panel-input', '1');
          return '[data-tt-panel-input="1"]';
        }
      }
      return null;
    });
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (sel) break;
    await sleep(1000);
    // bazen tiklamak gerekir
    await page.mouse.click(640, 800).catch(() => {});
  }

  if (!sel) {
    // son care: sayfada ne var logla
    ...
    throw new Error('mesaj kutusu yok | url=' + (dbg.url || '').slice(0, 40) + ' ce=' + dbg.ce + ' ta=' + dbg.ta);
  }
  ...
  if (typingMs > 0) {
    try {
      ...
      await sleep(Math.min(typingMs, 2500));
    } catch (_) {}
  }

  // 4) Metni yaz + gonder
  const typed = await page.evaluate(async (selector, msg) => {
    const input = document.querySelector(selector);
    if (!input) return { ok: false, error: 'input kayip' };
    input.focus();
    input.click();
    ...
    if (input.tagName === 'TEXTAREA' || input.tagName === 'INPUT') {
      ...
      const okInsert = document.execCommand('insertText', false, msg);
      if (!okInsert) {
  ...
  if (!typed?.ok) throw new Error(typed?.error || 'yazilamadi');
  ...
  // Send butonu
  await page.evaluate(() => {
    const btns = [...document.querySelectorAll('button, [role="button"], div[class*="Send"]')];
    for (const b of btns) {
      const t = (b.innerText || b.getAttribute('aria-label') || '').toLowerCase();
      const e2e = (b.getAttribute('data-e2e') || '').toLowerCase();
      if (/send|gonder|post/i.test(t + e2e) || e2e.includes('send')) {
        b.click();
        return;
      }
    }
