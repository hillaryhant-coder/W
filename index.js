...
async function applySession(page, sessionid) {
  ...
  await page.setCookie(base, { ...base, name: 'sessionid_ss' });
  await page.goto('https://www.tiktok.com/messages', { waitUntil: 'domcontentloaded', timeout: 90000 }).catch(() => {});
  await sleep(2500);
  const ok = await page.evaluate(() => document.cookie.includes('sessionid=')).catch(() => false);
  return ok;
}
...
async function addAccount(sessionid, name) {
  sessionid = String(sessionid || '').trim();
  if (!sessionid || sessionid.length < 10) throw new Error('sessionid gecersiz');
  ...
  const ok = await applySession(page, sessionid);
  if (!ok) {
    try { await browser.close(); } catch (_) {}
    delete live[id];
    store.accounts[id].status = 'error';
    save();
    throw new Error('sessionid ile giris basarisiz — cookie dogru mu?');
  }
...
app.post('/api/account/add', async (req, res) => {
  try {
    const r = await addAccount(req.body?.sessionid, (req.body?.name || '').trim());
    res.json(r);
  } catch (e) {
...
<div class="w">
  ...
  <div class="card">
    ...
    <p class="hint">Firefox/Chrome → tiktok.com giris → Cookie-Editor → sessionid kopyala. Oturum data/ icinde kalir.</p>
    <div id="accs"></div>
  </div>
